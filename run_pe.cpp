...
Eigen::VectorXd epl_for_each_Kup(Eigen::MatrixXi &mcmc, int loss_type) {
  ...
  for (int K = 1; K < N; K++) {
    ClusterEstimator cp(mcmc, static_cast<LOSS_FUNCTION>(loss_type), K,
                        initial_partition);
    epl_vec(K) = cp.expected_posterior_loss(cp.cluster_estimate(GREEDY));
  }
...
int main(int argc, char *argv[]) {
  ...
  std::cout << "Matrix with dimensions : " << mcmc.rows() << "*" << mcmc.cols()
            << " found." << std::endl;
  ...
  if (Kup == -1) {
    ...
    bayesmix::write_matrix_to_file(
        epl_for_each_Kup(mcmc, loss_type).transpose(), filename_out);
  }

  else if (Kup > 0) {
    Eigen::VectorXi initial_partition =
        Eigen::VectorXi::LinSpaced(mcmc.cols(), 1, mcmc.cols());
    ClusterEstimator cp(mcmc, static_cast<LOSS_FUNCTION>(loss_type), Kup,
                        initial_partition);
