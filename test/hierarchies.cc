...
TEST(mfahierarchy, sample_full_cond) {
  ...
  Eigen::VectorXd mutilde(0);
  // mutilde
  // << 1.1, 2.2, 3.1, 4.1, 4.8, 6.1, 6.9, 8.2, 9.0, 10.2, 11.2, 12.4, 13.5, 13.8,
  // 14.5, 14.5, 17.8 , 18.1, 19.0, 21;
  bayesmix::Vector mutilde_proto;
  bayesmix::to_proto(mutilde, &mutilde_proto);
  ...
  Eigen::MatrixXd data = bayesmix::read_eigen_matrix(
      "examples/mfa_hierarchy/in/samplefullcond.csv");
  ...
  *prior.mutable_fixed_values()->mutable_beta() = beta_proto;
  ...
  bayesmix::write_matrix_to_file(
      musamplato, "examples/mfa_hierarchy/out/musamplatofull.csv");
  bayesmix::write_matrix_to_file(
      psisamplato, "examples/mfa_hierarchy/out/psisamplatofull.csv");
  // bayesmix::write_matrix_to_file(Lambdasamplato,
  // "examples/mfa_hierarchy/out/Lambdasamplatofull1.csv");
  bayesmix::write_matrix_to_file(
      Lambdasamplato, "examples/mfa_hierarchy/out/Lambdasamplatofullt.csv");
  bayesmix::write_matrix_to_file(Etasamplato,
                                 "examples/mfa_hierarchy/out/Etasamplato.csv");
