...
//! Non Conjugate Normal Normal-InverseGamma hierarchy for univariate data.
...
//! contained in the Hypers object, are (mu0, sigma0, alpha0, beta0), all
//! scalar values. Note that this hierarchy is non conjugate.
...
class NNxIGHierarchy
    : public BaseHierarchy<NNxIGHierarchy, NNxIG::State, NNxIG::Hyperparams,
                           bayesmix::NNxIGPrior> {
 public:
  ...
  NNxIG::State draw(const NNxIG::Hyperparams &params);
  ...
  NNxIG::Hyperparams compute_posterior_hypers() const;
