...
double NWPriorModel::lpdf(const google::protobuf::Message &state_) {
  ...
  double target = stan::math::multi_normal_prec_lpdf(mean, hypers.mean, prec * hypers.var_scaling) + 
...
std::shared_ptr<google::protobuf::Message> NWPriorModel::sample(
  ...
  state.mutable_multi_ls_state()->mutable_prec_chol()->CopyFrom(prec_chol_proto);
...
void NWPriorModel::write_prec_to_state(const Eigen::MatrixXd &prec_, State::MultiLS *out) {
  out->prec = prec_;
