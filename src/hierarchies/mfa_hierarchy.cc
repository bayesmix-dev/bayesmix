...
double MFAHierarchy::like_lpdf(const Eigen::RowVectorXd & datum) const {
    ...
    return stan::math::normal_lpdf(
        datum, state.mu + state.Lambda*state.lambda.transpose(), state.psi.cwiseSqrt());
}
...
MFA::State MFAHierarchy::draw(const MFA::Hyperparams& params) {
    ...
    for (size_t j = 0; j < p; j++) {
        out.mu[j] = stan::math::normal_prec_rng(params.mutilde[j], 1 / sqrt(params.phi), rng);
...
void MFAHierarchy::initialize_state() {
    ...
    state.Eta = Eigen::MatrixXd::Zero(card,q);
    state.Lambda = Eigen::MatrixXd::Zero(p,q);
}
...
void MFAHierarchy::set_state_from_proto(
    ...
    state.mu = statecast.mfa_state().mu(); // TODO add, ls??
...
void MFAHierarchy::sample_full_cond(bool update_params = true) override {
    if (this->card == 0) {
...
void MFAHierarchy::sample_Eta(State &newstate) const {
    ...
    Eigen::MatrixXd Sigmaeta = (Eigen::MatrixXd::Identity(q, q) + state.Lambda.transpose() * Eigen::MatrixXd(psi.cwiseInverse()).asDiagonal() * state.Lambda).inverse();

    for (size_t i = 0; i < card; i++) {
        newstate.Eta.row(i) = stan::math::multi_normal_rng(Sigmaeta * state.Lambda.transpose() * Eigen::MatrixXd(psi.cwiseInverse()).asDiagonal() * (data[i] - state.mu), Sigmaeta, rng);
}
...
void MFAHierarchy::sample_mu(State &newstate) const {
    ...
    Eigen::MatrixXd Sigmamu = (hypers->phi*Eigen::MatrixXd::Identity(p, p) + card * Eigen::MatrixXd(psi.cwiseInverse()).asDiagonal()).inverse();
    ...
    newstate.mu = stan::math::multi_normal_rng(Sigmamu *(hypers->phi * hypers->mutilde + Eigen::MatrixXd(psi.cwiseInverse()).asDiagonal()*(data_sum-Somma)), Sigmamu, rng);
...
void MFAHierarchy::sample_Lambda(State &newstate) const {
    ...
    for (size_t j = 0; j < p; j++) {
        ...
        Eigen::MatrixXd Sigmalambda = (Eigen::MatrixXd::Identity(q, q) + state.Eta.transpose()/ state.psi[j] * state.Eta).inverse();

        newstate.Lambda.row(j) = stan::math::multi_normal_rng(Sigmalambda * state.Eta.transpose() / state.psi[j] * (data.col(j) - state.mu[j]), Sigmalambda, rng);
    }
...
void MFAHierarchy::sample_psi(State &newstate) const {
    ...
    for (size_t j = 0; j < p; j++) {
        ...
        double S=0;
        for (size_t i = 0; i < card; i++) {
            S += std::pow((data[i, j] - state.mu[j] - state.Lambda.row(j).dot(state.Eta.row(i))),2);
        }
        newstate.psi[j] = stan::math::inv_gamma_rng(hypers->alpha0 + card/2,hypers->beta[j] + S/2, rng);
    }
